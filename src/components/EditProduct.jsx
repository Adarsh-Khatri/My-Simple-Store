...
import { getApi, putApi } from '../services/httpServices';

export default class EditProduct extends Component {

  state = {
    product: { id: '', category: '', description: '', imgLink: '', name: '', price: '' }, categoryArr: ["Watches", "Sunglasses", "Belts", "Wallets", "Handbags", "Formal Shoes", "Sports Shoes", "Floaters", "Sandals"], errors: {}
  }

  fetchData = async () => {
    try {
      let { id } = this.props.match.params;
      let product = await getApi(`/getproducts/${id}`);
  ...
  render() {
    ...
    let { category, description, imgLink, name, price } = this.state.product;
    let { id } = this.props.match.params;

    return (
      <div className="container">
        <div className="row">
          <h1 className='text-center fw-bold'>Edit Product</h1>
        </div>
        <div className="row">
          <div className="col-sm-4">{this.showProductImage(product)}</div>
          <div className='col-sm-8 bg-light rounded rounded-5 p-5'>
            ...
            <div className='mt-3 d-flex'>
              <div className='d-flex justify-content-center w-100 gap-5'>
                <button type='button' className='btn btn-primary' disabled={!this.isFormValid()} onClick={(e) => this.handleSubmit(e)}>Save</button>
                <Link className="btn btn-danger" to={`/product/delete/${id}`}>Delete</Link>
              </div>
